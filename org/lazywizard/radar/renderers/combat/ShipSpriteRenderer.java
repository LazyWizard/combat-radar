    ...
    public void render(ShipAPI player, float amount, boolean isUpdateFrame)
    {
        if (SHOW_SHIPS && player.isAlive())
        {
            ...
            if (!contacts.isEmpty())
            {
                ...
                {
                    SpriteAPI sprite = entity.getSpriteAPI();
                    int playerSide = player.getOwner();
                    float alphaMod = (entity.getPhaseCloak() != null
                            && entity.getPhaseCloak().isOn()) ? PHASE_ALPHA_MULT : 1f;
                    sprite.setAlphaMult(radar.getContactAlpha() * alphaMod);

                    // Hulks
                    if (entity.isHulk())
                    {
                        sprite.setColor(radar.getNeutralContactColor());
                    }
                    // Allies
                    else if (entity.getOwner() == playerSide)
                    {
                        sprite.setColor(radar.getFriendlyContactColor());
                    }
                    // Enemies
                    else if (entity.getOwner() + playerSide == 1)
                    {
                        sprite.setColor(radar.getEnemyContactColor());
                    }
                    ...
                    float sizeMult = radar.getCurrentPixelsPerSU();
                    Vector2f radarLoc = radar.getPointOnRadar(entity.getLocation());
                    //System.out.println("Rendering "+entity.getVariant().getHullVariantId()
                    //        +" at " + radarLoc + ", size mult " + sizeMult);
                    sprite.setNormalBlend();
                    sprite.setSize(sprite.getWidth() * sizeMult, sprite.getHeight() * sizeMult);
                    sprite.renderAtCenter(radarLoc.x, radarLoc.y);
